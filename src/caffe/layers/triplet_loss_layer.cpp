...
void TripletLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
    ...
    LOG(INFO) << "Triplet loss group num is " << group_num;
...
template <typename Dtype>
void TripletLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
...
template <typename Dtype>
void TripletLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
    ...
    for(int i=0; i<group_num; ++i) 
	{
        for(int j=1; j<N; ++j) 
		{
            if(label_ptr[i*N]==label_ptr[i*N+j]) 
        ...
        if(pos_ids[i].size()==0 || neg_ids[i].size()==0) 
			continue;
    ...
    valid_triplet_group = cnt;
    loss = loss / (cnt + 0.00000001) / Dtype(2);
    top[0]->mutable_cpu_data()[0] = loss;
	top[0]->mutable_cpu_data()[1] = valid_triplet_group;
}

template <typename Dtype>
void TripletLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) 
{
    Dtype alpha;
	if(valid_triplet_group == 0)
    	alpha = top[0]->cpu_diff()[0]/ (valid_triplet_group+ 0.000001);
    else
    	alpha = top[0]->cpu_diff()[0]/ (valid_triplet_group);
